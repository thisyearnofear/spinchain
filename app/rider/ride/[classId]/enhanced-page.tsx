// Enhanced Live Ride Page with BLE Integration
// Following Core Principles: Enhancement First, DRY, CLEAN separation
...
import { DeviceSelector } from "../../../../components/ble/device-selector";
import { useDeviceType, useOrientation, useActualViewportHeight } from "../../../../lib/responsive";
import { motion, AnimatePresence } from "framer-motion";

/**
 * Enhanced Live Ride Page with BLE Integration
 * ENHANCEMENT FIRST: Builds upon existing ride page with real device data
 * DRY: Reuses existing responsive and UI patterns
 * MODULAR: Composable BLE integration
 */

export default function EnhancedLiveRidePage() {
  ...
  const {
    ...
    autoConnect: false,
    onSuccess: (metrics) => {
      // Update telemetry with real BLE data when available
      setTelemetry(prev => ({
        ...prev,
  ...
  // Telemetry - Enhanced with BLE data fallback
  ...
  // Simulate ride progress with real BLE data integration
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Use BLE data when available, fallback to simulation
      if (!bleConnected) {
        setTelemetry({
      ...
      // When BLE is connected, data is updated via the useBleData callback
    }, 1000);

    return () => clearInterval(interval);
  }, [isRiding, classData, bleConnected]);

  // Calculate effort based on real metrics
  const calculatedEffort = useMemo(() => {
    if (bleConnected && bleMetrics) {
      // Real effort calculation based on power and heart rate
      const powerEffort = Math.min((bleMetrics.power / 300) * 100, 100);
      const hrEffort = Math.min(((bleMetrics.heartRate - 60) / 140) * 100, 100);
      return Math.round((powerEffort + hrEffort) / 2);
    }
    return telemetry.effort;
  }, [bleConnected, bleMetrics, telemetry.effort]);
    ...
    >
      ...
      {showHUD && (
        <div className="absolute inset-0 pointer-events-none">
          {/* Top Bar - Enhanced with BLE Status */}
              ...
              <div className="flex items-center gap-2 ml-2">
                ...
                <div className="flex items-center gap-1">
                  <div className={`h-2 w-2 rounded-full ${
                    ...
                    blePending ? 'bg-yellow-500 animate-pulse' : 'bg-gray-500'
                  }`} />
                  <span className="text-xs text-white/50 hidden sm:inline">
                    {bleConnected ? 'Connected' : blePending ? 'Connecting...' : 'Disconnected'}
                  </span>
                </div>
          ...
          {/* Center - Telemetry (Responsive Layout with BLE Integration) */}
          {hudMode !== "minimal" && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none p-3 sm:p-6">
              {/* Mobile: Single Column with BLE Status */}
              {deviceType === "mobile" && hudMode === "compact" && (
                <div className="flex flex-col gap-2 w-full max-w-[200px]">
                  {/* Primary Metric - Heart Rate with BLE indicator */}
                  <div className="rounded-xl bg-black/70 backdrop-blur-xl border border-white/20 p-4 text-center relative">
                    {bleConnected && (
                    ...
                    {bleConnected && (
                      <p className="text-[8px] text-green-400 mt-1">LIVE</p>
                    )}
                  </div>
              ...
              {/* Desktop/Tablet: Full Grid with Enhanced Metrics */}
              {(deviceType === "desktop" || (deviceType === "tablet" && orientation === "landscape")) && hudMode === "full" && (
                <div className="grid grid-cols-2 gap-4 sm:gap-6">
                  <div className="rounded-2xl bg-black/60 backdrop-blur-xl border border-white/20 p-4 sm:p-6 min-w-[160px] sm:min-w-[180px] relative">
                    {bleConnected && (
                      <div className="absolute -top-2 -right-2 h-4 w-4 rounded-full bg-green-500 border-2 border-black animate-pulse" />
                    )}
                  ...
                  <div className="rounded-2xl bg-black/60 backdrop-blur-xl border border-white/20 p-4 sm:p-6 min-w-[160px] sm:min-w-[180px]">
                    <p className="text-xs uppercase tracking-wider text-white/50 mb-2">Effort</p>
                    <p className="text-4xl sm:text-5xl font-bold text-purple-400">
                      {calculatedEffort}
                      <span className="text-lg sm:text-xl text-white/50 ml-2">%</span>
                    </p>
                  </div>
                </div>
              )}
          ...
          {/* Bottom - Controls with Device Integration */}
              ...
              {hudMode !== "minimal" && (
                <div className="flex items-center justify-between mb-3 sm:mb-4 text-white">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xl sm:text-2xl font-bold text-purple-400">{calculatedEffort}</p>
                  </div>
                </div>
              )}
              
              {/* Controls - Enhanced with BLE connection state */}
              <div className="flex items-center justify-center gap-3">
                {!isRiding ? (
                  <button
                    onClick={startRide}
                    disabled={blePending}
                    className={`rounded-full px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg font-semibold shadow-lg transition-all active:scale-95 touch-manipulation min-h-[56px] ${
                      blePending 
                        ? 'bg-gray-600 text-gray-400 cursor-not-allowed'
                        : 'bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-indigo-500/50'
                    }`}
                  >
                    {blePending ? 'Connecting...' : rideProgress > 0 ? "Resume" : "Start Ride"}
                  </button>
                ) : (
                  <button
                    onClick={pauseRide}
      ...
      {/* Device Panel - Slide-in panel for device management */}
      <AnimatePresence>
        {showDevicePanel && (
          <motion.div
            initial={{ x: "100%" }}
            ...
            className="absolute top-0 right-0 h-full w-full max-w-md bg-black/95 backdrop-blur-xl border-l border-white/20 pointer-events-auto"
          >
            <div className="p-6 h-full overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-white">Device Connection</h2>
                <button
              ...
              <DeviceSelector 
                onMetricsUpdate={(metrics) => {
                  // Real-time metrics updates handled by useBleData hook
                }}
              />
              
              {/* Device Info */}
              {bleConnected && bleMetrics && (
                <div className="mt-6 p-4 rounded-lg bg-white/5 border border-white/10">
                  <h3 className="text-sm font-semibold text-white mb-3">Live Metrics</h3>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <p className="text-white/50">Power</p>
                      <p className="text-white font-medium">{bleMetrics.power} W</p>
                    </div>
                    <div>
                      <p className="text-white/50">Cadence</p>
                      <p className="text-white font-medium">{bleMetrics.cadence} RPM</p>
                    </div>
                    <div>
                      <p className="text-white/50">Heart Rate</p>
                      <p className="text-white font-medium">{bleMetrics.heartRate} BPM</p>
                    </div>
                    <div>
                      <p className="text-white/50">Last Update</p>
                      <p className="text-white/70 text-xs">
                        {new Date(bleMetrics.timestamp).toLocaleTimeString()}
                      </p>
                    </div>
                  </div>
                </div>
