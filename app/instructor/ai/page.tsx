    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#1a2550,transparent_55%),radial-gradient(circle_at_80%_20%,#2a1d5a,transparent_40%)]">
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 pb-20 pt-10 lg:px-12">
        ...
        <div className="flex flex-col gap-2">
            ...
            <h1 className="text-4xl font-bold text-white">AI Instructor Studio</h1>
            ...
            <div className="flex flex-col gap-6">
                <GlassCard className="p-8">
                    ...
                    <div className="mt-6 grid gap-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-white/80">Coaching Personality</label>
                            <div className="grid grid-cols-3 gap-3">
                                {[
                                    { id: "zen", label: "Zen Master", icon: "🧘" },
                                    { id: "drill-sergeant", label: "Drill Sergeant", icon: "⚡" },
                    ...
                    <div className="mt-6 space-y-4">
                        <div className="flex items-start gap-4 rounded-xl border border-white/10 bg-white/5 p-4">
                            <div className="mt-1 grid h-8 w-8 place-items-center rounded-full bg-blue-500/20 text-blue-300">
                                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 19.7778H22L12 2Z" /></svg>
                            </div>
                            <div>
                                <h4 className="font-semibold text-white">Base (Settlement Layer)</h4>
                                <p className="text-sm text-white/60">Handles high-value assets: Tickets (NFTs), $SPIN rewards, and ENS identity.</p>
                            </div>
                        </div>

                        <div className={`flex items-start gap-4 rounded-xl border p-4 transition-all ${
                            suiEnabled ? "border-cyan-500/30 bg-cyan-500/10" : "border-white/10 bg-white/5 opacity-50"
                        }`}>
                            ...
                            <div className="flex-1">
                                <div className="flex items-center justify-between">
                                    <h4 className="font-semibold text-white">Sui (Performance Layer)</h4>
                                    <div className="relative inline-flex h-5 w-9 cursor-pointer items-center rounded-full bg-white/20" onClick={() => setSuiEnabled(!suiEnabled)}>
                                        <span className={`h-3 w-3 rounded-full bg-white shadow-sm transition-transform ${suiEnabled ? "translate-x-5" : "translate-x-1"}`} />
                                    </div>
                                </div>
                                <p className="text-sm text-white/60 mt-1">
                                    Processes real-time biometrics (10Hz), adjusts music/lighting agents, and stores route telemetry cheaply.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex flex-col gap-6">
                    ...
                    <div className="bg-gradient-to-br from-indigo-900 via-purple-900 to-black p-8 text-center">
                        <div className="mx-auto h-24 w-24 overflow-hidden rounded-full border-4 border-white/10 shadow-2xl">
                             <div className="grid h-full w-full place-items-center bg-black text-4xl">
                                {personality === 'zen' ? '🧘' : personality === 'drill-sergeant' ? '⚡' : '📊'}
                             </div>
                        </div>
                        <h3 className="mt-4 text-2xl font-bold text-white">{agentName}</h3>
                        <p className="text-indigo-200">AI Instructor • Level 1</p>
                    </div>
                    <div className="p-6">
                        <h4 className="text-xs uppercase tracking-widest text-white/50">Capabilities</h4>
                        <div className="mt-4">
                ...
                >
                    <p className="mb-4 text-sm text-white/60">
                        This agent can automatically manage liquidity for your class tokens based on attendance demand.
                    </p>
