...
describe("SuiEngine", () => {
  ...
  describe("reward minting", () => {
    it("should mint a single SPIN reward", async () => {
      const { engine } = createEngine();
      setActiveSession(engine);

      const result = await engine.mintReward(1000, "Completed endurance ride");
      expect(result).toBe(true);

      expect(mockMoveCall).toHaveBeenCalledWith(
        expect.objectContaining({
          target: expect.stringContaining("mint_reward"),
        }),
      );
    });

    it("should accept a recipient address for mintReward", async () => {
      const { engine } = createEngine();
      setActiveSession(engine);

      const result = await engine.mintReward(
        1000,
        "Ride reward",
        "0xrecipient",
      );
      ...
      expect(mockPure.address).toHaveBeenCalledWith("0xrecipient");
    });

    it("should batch mint rewards", async () => {
      const { engine } = createEngine();
      setActiveSession(engine);

      const recipients = ["0xalice", "0xbob"];
      const amounts = [500, 750];
      const result = await engine.batchMintRewards(
        recipients,
        amounts,
        "Class rewards",
      );
      ...
      expect(mockMoveCall).toHaveBeenCalledWith(
        expect.objectContaining({
          target: expect.stringContaining("batch_mint_rewards"),
        }),
      );
      expect(mockPure.vector).toHaveBeenCalledWith(
        "address",
        expect.arrayContaining(["0xalice", "0xbob"]),
      );
    });
    ...
    it("should reject minting without treasury cap", async () => {
      const { engine } = createEngine({ treasuryCapId: "" });
      setActiveSession(engine);

      const result = await engine.mintReward(1000, "Test");
      expect(result).toBe(false);
    });
  });
